...
for (hash_value in hash_values) {
    write.table(clones_sample_data[clones_sample_data$HashValue == hash_value,c("Content")], file=paste0("export/", hash_value, "_content.csv"), sep=",", col.names=TRUE, row.names=FALSE, na="", quote=TRUE, qmethod="double", fileEncoding="UTF-8")
}
...
write.table(clones_sample_data[order(-clones_sample_data$ThreadCount, -clones_sample_data$LineCount), c("HashValue", "ThreadCount", "LineCount")],
            file="export/index.csv", sep=",", col.names=TRUE, row.names=FALSE, na="", quote=TRUE, qmethod="double", fileEncoding="UTF-8")
...
for (hash_value in hash_values) {
  ...
  write.table(so_links, file=paste0("export/", hash_value, "_so-links.csv"), sep=",", col.names=TRUE, row.names=FALSE, na="", quote=TRUE, qmethod="double", fileEncoding="UTF-8")
}
